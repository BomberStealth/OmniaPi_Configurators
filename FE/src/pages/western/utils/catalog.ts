...
export interface InverterEntry {
  ...
  catalogCode: string;    // codice prodotto WHi (catalogo manifatturiero)
  desc: string;
}
...
function inv(
  ...
  return { id, phase, wtype, series, powerKw, mppt, battVoltage, maxBattV, label, prefix: 'WST', catalogCode, desc };
}

// ─────────────────────────────────────────────────────────────────────────────
// Catalogo WHi 2025 — codici prodotto manifatturiere (6 cifre) da pag. 7-31
// ─────────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    if (raw) {
      const saved = JSON.parse(raw) as {
        inverters?: { id: string; prefix: string; catalogCode: string }[];
        batteries?: { id: string; prefix: string; catalogCode: string }[];
        accessories?: { id: string; prefix: string; catalogCode: string }[];
      };
      return {
        inverters: DEFAULT_CATALOG.inverters.map(i => {
          const s = saved.inverters?.find(x => x.id === i.id);
          return s ? { ...i, prefix: s.prefix, catalogCode: s.catalogCode ?? i.catalogCode } : i;
        }),
        batteries: DEFAULT_CATALOG.batteries.map(b => {
          const s = saved.batteries?.find(x => x.id === b.id);
          return s ? { ...b, prefix: s.prefix, catalogCode: s.catalogCode ?? b.catalogCode } : b;
        }),
        accessories: DEFAULT_CATALOG.accessories.map(a => {
          const s = saved.accessories?.find(x => x.id === a.id);
          return s ? { ...a, prefix: s.prefix, catalogCode: s.catalogCode ?? a.catalogCode } : a;
        }),
      };
    }
  } catch { /* ignore */ }
...
export function saveCatalog(cat: WCatalog): void {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({
    inverters:   cat.inverters.map(i => ({ id: i.id, prefix: i.prefix, catalogCode: i.catalogCode })),
    batteries:   cat.batteries.map(b => ({ id: b.id, prefix: b.prefix, catalogCode: b.catalogCode })),
    accessories: cat.accessories.map(a => ({ id: a.id, prefix: a.prefix, catalogCode: a.catalogCode })),
  }));
}
